...
-- 300 -> (05:00)
-- 3600 -> (01:00:00)
function Module.FormatForTimer(seconds, slice_hours)
	...
	if slice_hours and hours == 0 then
...
function Module.FromRomanNumeral(Numeral)
	...
	local Count = 1
	for i=1, #Numeral do
		local Letter = string.sub(Numeral, i, i)
		if not RomanDigit[Letter] then
			return nil
		end
		if Letter == Last then
			Count = Count + 1
	...
	while Pointer <= #Numeral do
		local Letter = string.sub(Numeral, Pointer, Pointer)
		local Digit = RomanDigit[Letter]
		if Digit > MaxDigit then
			return nil
		end
		local NextDigit = 0
		if Pointer <= #Letter - 1 then
			local NextNumeral = string.sub(Letter, Pointer+1, Pointer+1)
			NextDigit = RomanDigit[NextNumeral]
			if NextDigit > Digit then
				if (not SpecialRomanDigit[Letter]) or NextDigit > (Digit * 10) or CheckNumOfCharacterInString(Letter, Letter) > 3 then
					return nil
				end
				MaxDigit = Digit - 1
